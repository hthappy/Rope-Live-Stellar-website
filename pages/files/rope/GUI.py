...
from rope.Dicts import CAMERA_BACKENDS, get_media_path
from rope.FaceLandmarks import FaceLandmarks
from rope.FaceEditor import FaceEditor
from rope.rtmp_sniffer import get_base_dir
import rope.OBS_NDI_setup as OBS_NDI_setup
import threading
import gc
from obswebsocket import obsws, requests
from rope.license_utils import check_license, get_remaining_days
from rope.rtmp_sniffer import get_rtmp_info
import simpleobsws
import asyncio
import subprocess

from rope.version import version_checker, VERSION
import logging

class GUI(tk.Tk):
    def __init__(self, models, media_path=None):
        super().__init__()
        self.models = models
        if media_path is None:
            media_path = os.path.join(os.path.dirname(__file__), 'media')
        self.media_path = media_path
        self.image_cache = {}  # 用于缓存图像
        self.title('LiveFaceX - AI换脸工具')
        self.target_media = []
        ...
        #从上一级的配置文件夹中读取 config.json 文件中的 OBS WebSocket 配置
        config_path = os.path.join(get_base_dir(), 'config', 'config.json')
        with open(config_path, 'r', encoding='utf-8') as file:
            config = json.load(file)
        self.obs_host = config.get("obs", {}).get("host", "")
        ...
        self.key_actions = {
            shortcuts["时间轴开始"]: lambda: self.preview_control('q'),
            shortcuts["向左轻移30帧"]: lambda: self.preview_control('a'),
            shortcuts["记录"]: lambda: self.toggle_rec_video(),
            shortcuts["Play"]: lambda: self.toggle_play_video(),
            shortcuts["右移30帧"]: lambda: self.preview_control('d'),
            shortcuts["保存图像"]: lambda: self.save_image(),
            shortcuts["添加标记"]: lambda: self.update_marker('add'),
            shortcuts["删除标记"]: lambda: self.update_marker('delete'),
            shortcuts["上一个标记"]: lambda: self.update_marker('prev'),
            shortcuts["下一个标记"]: lambda: self.update_marker('next'),
            shortcuts["切换恢复"]: lambda: self.toggle_and_update('Restorer', 'Restorer'),
            shortcuts["切换恢复2"]: lambda: self.toggle_and_update('Restorer2', 'Restorer2'),
            shortcuts["切换方向"]: lambda: self.toggle_and_update('Orient', 'Orientation'),
            shortcuts["切换强度"]: lambda: self.toggle_and_update('Strength', 'Strength'),
            shortcuts["切换差异"]: lambda: self.toggle_and_update('Diff', 'Differencing'),
            shortcuts["切换遮挡"]: lambda: self.toggle_and_update('Occluder', 'Occluder'),
            shortcuts["切换人脸解析"]: lambda: self.toggle_and_update('FaceParser', 'Face Parser'),
            shortcuts["切换基于文本的蒙版"]: lambda: self.toggle_and_update('CLIP', 'Text-Based Masking'),
            shortcuts["切换颜色调整"]: lambda: self.toggle_and_update('Color', 'Color Adjustments'),
            shortcuts["切换面部调整"]: lambda: self.toggle_and_update('FaceAdj', 'Input Face Adjustments'),
            shortcuts["清除 VRAM"]: lambda: self.clear_mem(),
            shortcuts["Swap Faces（换脸）"]: lambda: self.toggle_swapper(),
            shortcuts["向左轻移1帧"]: lambda:self.back_one_frame(),
            shortcuts["右移1帧"]: lambda: self.forward_one_frame(),
            shortcuts["显示蒙版"]: lambda: self.toggle_maskview(),
        }
            ...
            try:
                if version_checker.check_for_updates():
                    ...
                    self.version_label.config(
                        text=f"当前版本: {VERSION} (发现新版本)",
                        cursor="hand2"
                    )
                    self.version_label.bind(
                        "<Button-1>", 
                        lambda e: version_checker.show_update_dialog()
                    )
                else:
                    self.version_label.config(
                        text=f"当前版本: {VERSION}",
        ...
        def periodic_update_check():
            ...
            # 每24小时检查一次
            self.after(24 * 60 * 60 * 1000, periodic_update_check)
        
        # 启动定期检查
        self.after(1000, periodic_update_check)
    ...
    def update_license_status(self):
        if check_license():
            ...
            status_text = f"软件已激活，剩余使用天数：{remaining_days}"
        else:
            status_text = "软件未激活，请激活"
        
        self.license_status_label.config(text=status_text)

    #    工具菜单选择
    def show_live_menu(self):
        if self.live_menu is None:
            self.live_menu = tk.Menu(self, tearoff=0)
            self.live_menu.add_command(label="获取推流码", command=lambda: self.on_live_select("获取推流码"))
            self.live_menu.add_command(label="OBS自动推流", command=lambda: self.on_live_select("OBS自动推流"))
            self.live_menu.add_command(label="NDI一键配置", command=lambda: self.on_live_select("NDI一键配置"))
            self.live_menu.add_separator()
            self.live_menu.add_command(
                label="检查更新", 
                command=lambda: self.check_and_show_update()
            )

        # 显示菜单
        self.live_menu.post(self.live_button.winfo_rootx(), self.live_button.winfo_rooty() + self.live_button.winfo_height())

    def check_npcap(self):
        try:
            subprocess.run(["sc", "query", "npcap"], check=True, capture_output=True)
            return True
        except subprocess.CalledProcessError:
            return False

    def on_live_select(self, choice):
        if choice == "获取推流码":
            #检查是否安装了Npcap
            if not self.check_npcap():
                #如果未安装，则显示错误信息并点击确定跳转到https://npcap.com/#download
                print("Npcap 未安装。请访问 https://npcap.com 下载并安装 Npcap。")
                messagebox.showerror("错误", "Npcap 未安装。请访问 https://npcap.com 下载并安装 Npcap。")
    ...
    def get_stream_key(self):
        try:
            ...
            if server and code:
                full_rtmp = f"{server}/{code}"
        except Exception as e:
            messagebox.showerror("错误", f"获取推流信息时发生错误：{str(e)}")

    async def configure_obs_for_douyin(self, rtmp_url):
        ...
        try:
            ...
            # 获取当前的流服务设置
            get_current_settings_request = simpleobsws.Request('GetStreamServiceSettings')
            get_current_settings_response = await ws.call(get_current_settings_request)
            if get_current_settings_response.ok():
                print("当前的流服务设置:", json.dumps(get_current_settings_response.responseData, indent=2))
            else:
                print("获取当前流服务设置失败:", get_current_settings_response.responseData)

            server, stream_key = rtmp_url.split('/third/', 1)
            server = f"{server}/third"
            
            new_settings = {
                "server": server,
                "key": stream_key
            }
            print("尝试设置新的流服务设置:", json.dumps(new_settings, indent=2))

            request = simpleobsws.Request('SetStreamServiceSettings', {
                'streamServiceType': 'rtmp_custom',
                'streamServiceSettings': new_settings
            })
            ...
            if response.ok():
                ...
                print("应用新的流服务设置失败:", response.responseData)

            # 获取更新后的设置
            get_request = simpleobsws.Request('GetStreamServiceSettings')
            get_response = await ws.call(get_request)
            if get_response.ok():
                updated_settings = get_response.responseData
                print("更新后的流服务设置:", json.dumps(updated_settings, indent=2))

                if (updated_settings['streamServiceSettings']['server'] == server and 
                    updated_settings['streamServiceSettings']['key'] == stream_key):
                    self.after(0, lambda: messagebox.showinfo("成功", "OBS推流设置成功，开始直播吧。"))
                else:
                    self.after(0, lambda: messagebox.showwarning("警告", "OBS推流设置可能未成功更新，请手动检查OBS设置"))
            else:
                print("获取更新后的设置失败:", get_response.responseData)

        except Exception as e:
            error_message = f"设置OBS时出错：{str(e)}"
            print(error_message)
            self.after(0, lambda: messagebox.showerror("错误", error_message))
        finally:
            await ws.disconnect()
            print("OBS推流设置完成，请开始你的直播吧！")
    ...
    def start_stream(self):
        try:
            ...
            if server and code:
                full_rtmp = f"{server}/{code}"
                self.loop.run_until_complete(self.configure_obs_for_douyin(full_rtmp))
            else:
                messagebox.showinfo("错误", "未能获取到完整的推流信息。请确保直播伴侣已打开。")
        except Exception as e:
            messagebox.showerror("错误", f"配置OBS时发生错误：{str(e)}")
...
#####
    def create_gui(self):
        ...
        save_ks_button = ctk.CTkButton(ks_frame, text="保存快捷键", command=save_shortcuts, width=150, height=15, corner_radius=3, fg_color=style.main2, hover_color=style.main3)
        ...
        tab_face_swapper_visibility = tabview_main_visibility.add("换脸参数")
        tab_face_editor_visibility = tabview_main_visibility.add("编辑人脸")
        ...
        save_pv_button = ctk.CTkButton(button_frame, text="保存", command=save_params_visibility, width=80, height=15, corner_radius=3, fg_color=style.main, hover_color=style.main3, text_color="#FFFFE0", anchor='center')
        save_pv_button.grid(row=0, column=0, padx=0, pady=0)

        apply_pv_button = ctk.CTkButton(button_frame, text="应用", command=lambda: apply_params_visibility_configuration(params_visibility=None, params_face_editor_visibility=None, param_type='all', reload=False), width=80, height=15, corner_radius=3, fg_color=style.main, hover_color=style.main3, text_color="#FFFFE0", anchor='center')
        apply_pv_button.grid(row=0, column=1, padx=0, pady=0)

        load_pv_button = ctk.CTkButton(button_frame, text="加载", command=load_params_visibility_configuration, width=80, height=15, corner_radius=3, fg_color=style.main, hover_color=style.main3, text_color="#FFFFE0", anchor='center')
        load_pv_button.grid(row=0, column=2, padx=0, pady=0)

        default_pv_button = ctk.CTkButton(button_frame, text="缺省", command=default_params_visibility_configuration, width=80, height=15, corner_radius=3, fg_color=style.main, hover_color=style.main3, text_color="#FFFFE0", anchor='center')
...
####### Top Frame
      ...
      # Right
        ...
        # 添加工具按钮
        self.live_button = tk.Button(self.layer['topright'], text="工具", command=self.show_live_menu, width=6, height=1, bg='#151515', fg='#b0b0b0')
        self.live_button.place(x=20, y=9)
        ...
        self.checkbox = ctk.CTkCheckBox(self.layer['topleft'], text="输入面板", text_color='#B0B0B0', command=input_panel_checkbox, onvalue=True, offvalue=False, checkbox_width=18, checkbox_height=18, border_width=0, hover_color='#303030', fg_color=style.main)
        self.checkbox.place(x=500, y=10)
        self.checkbox.select()
        #Button - Hide/Unhide Inputs Panel
        self.collapse_bottom = ctk.CTkCheckBox(self.layer['topleft'], text="脸部面板",text_color='#B0B0B0', command=collapse_faces_panel, onvalue=True, offvalue=False, checkbox_width=18,checkbox_height=18,border_width=0,hover_color='#303030',fg_color=style.main)
        self.collapse_bottom.place(x=600, y=10)
        self.collapse_bottom.select()
        #Button - Hide/Unhide Params Panel
        self.collapse_params = ctk.CTkCheckBox(self.layer['topleft'], text="参数面板",text_color='#B0B0B0', command=collapse_params_panel, onvalue=True, offvalue=False, checkbox_width=18,checkbox_height=18,border_width=0,hover_color='#303030',fg_color=style.main)
        self.collapse_params.place(x=705, y=10)
        self.collapse_params.select()
        #Button - Hide/Unhide Keyboard Shortcuts Panel
        self.collapse_keyboardshortcuts = ctk.CTkCheckBox(self.layer['topleft'], text="键盘快捷键",text_color='#B0B0B0', command=keyboard_shortcuts, onvalue=True, offvalue=False, checkbox_width=18,checkbox_height=18,border_width=0,hover_color='#303030',fg_color=style.main)
        self.collapse_keyboardshortcuts.place(x=840, y=10)
        #Button - Hide/Unhide Parameters Visibility Panel
        self.collapse_parametersvisibility = ctk.CTkCheckBox(self.layer['topleft'], text="参数可视化",text_color='#B0B0B0', command=parameters_visibility, onvalue=True, offvalue=False, checkbox_width=18,checkbox_height=18,border_width=0,hover_color='#303030',fg_color=style.main)
...
####### Middle Frame
    ...
    ### Videos and Faces
      ...
      # Input Videos
        ...
        self.target_media_canvas.create_text(8, 20, anchor='w', fill='grey25', font=("Microsoft YaHei", 20), text=" 要换脸的视频")
      ...
      # Input Faces
        ...
        self.source_faces_canvas.create_text(8, 20, anchor='w', fill='grey25', font=("Microsoft YaHei", 20), text="要换成头像")
    ...
    # FaceLab
      ...
      # Found Faces
        ...
        self.found_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Microsoft YaHei", 20), text="找到的人脸")
      ...
      # Merged Faces
        ...
        self.merged_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Microsoft YaHei", 20), text=" 脸模型文件（DFM）")
    ...
    ### Parameters
        ...
        # Aggiungi Tabs al CTkTabview
        tab_face_swapper = tabview_main.add("换脸参数设置")
        tab_live_portrait = tabview_main.add("编辑人脸设置")
        ...
        # Providers Priority
        row = row + 1
        self.widget['ProvidersPriorityTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'ProvidersPriorityTextSel', '计算内核', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72)
        row = row + 1
        self.widget['ThreadsSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ThreadsSlider', '线程数量', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.72)

        # Face Swapper Model
        row = row + 1
        self.widget['FaceSwapperModelTextSel'] = GE.TextSelectionComboBox(self.layer['parameters_frame'], 'FaceSwapperModelTextSel', '换脸模型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72, 150)
        row = row + 1
        self.widget['SwapperTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'SwapperTypeTextSel', '换脸像素', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72)

        #Webcam Backend
        row = row + 1
        self.widget['WebCamBackendSel'] = GE.TextSelectionComboBox(self.layer['parameters_frame'], 'WebCamBackendSel', '摄像头后端', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72, 150)
        ...
        #Webcam Max FPS
        row = row + 1
        self.widget['WebCamMaxFPSSel'] = GE.TextSelection(self.layer['parameters_frame'], 'WebCamMaxFPSSel', '摄像头每秒帧数', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72)

        #Webcam Max Count
        row = row + 1
        self.widget['WebCamMaxNoSlider'] = GE.Slider2(self.layer['parameters_frame'], 'WebCamMaxNoSlider', '摄像头最大数量', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.72)

        #Virtual Cam
        row = row + 1
        self.widget['VirtualCameraSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'VirtualCameraSwitch', '发送至虚拟摄像头', 3, self.toggle_virtualcam, 'control', 398, 20, row, 0, padx, pady)

        # Restore
        row = row + 1
        self.widget['RestorerSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'RestorerSwitch', '高清修复', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['RestorerTypeTextSel'] = GE.TextSelectionComboBox(self.layer['parameters_frame'], 'RestorerTypeTextSel', '修复模型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72, 150)
        row = row + 1
        self.widget['RestorerDetTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'RestorerDetTypeTextSel', '检测对齐', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72)
        row = row + 1
        self.widget['VQFRFidelitySlider'] = GE.Slider2(self.layer['parameters_frame'], 'VQFRFidelitySlider', '保真度比率', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.72)
        row = row + 1
        self.widget['RestorerSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestorerSlider', '混合度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.72)

        # Restore2
        row = row + 1
        self.widget['Restorer2Switch'] = GE.Switch2(self.layer['parameters_frame'], 'Restorer2Switch', '2. 高清修复', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['Restorer2TypeTextSel'] = GE.TextSelectionComboBox(self.layer['parameters_frame'], 'Restorer2TypeTextSel', '2. 修复模型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72, 150)
        row = row + 1
        self.widget['Restorer2DetTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'Restorer2DetTypeTextSel', '2. 检测对齐', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72)
        row = row + 1
        self.widget['Restorer2Slider'] = GE.Slider2(self.layer['parameters_frame'], 'Restorer2Slider', '2. 混合度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.72)

        # Frame Restorer
        row = row + 1
        self.widget['FrameEnhancerTypeTextSel'] = GE.TextSelectionComboBox(self.layer['parameters_frame'], 'FrameEnhancerTypeTextSel', '帧类型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.72, 150)
        row = row + 1
        self.widget['EnhancerSlider'] = GE.Slider2(self.layer['parameters_frame'], 'EnhancerSlider', '帧混合度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.72)

        # Orientation
        row = row + 1
        self.widget['OrientSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'OrientSwitch', '方向', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['OrientSlider'] = GE.Slider2(self.layer['parameters_frame'], 'OrientSlider', '角度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Strength
        row = row + 1
        self.widget['StrengthSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'StrengthSwitch', '强度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['StrengthSlider'] = GE.Slider2(self.layer['parameters_frame'], 'StrengthSlider', '数值', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Border
        row = row + 1
        self.widget['BorderTopSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderTopSlider', '顶部边框距离', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['BorderLeftSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderLeftSlider', '左侧边框距离', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['BorderRightSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderRightSlider', '右侧边框距离', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['BorderBottomSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderBottomSlider', '底部边框距离', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['BorderBlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderBlurSlider', '边框模糊', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Diff
        row = row + 1
        self.widget['DiffSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'DiffSwitch', '差异化', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['DiffSlider'] = GE.Slider2(self.layer['parameters_frame'], 'DiffSlider', '数值', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['DiffingBlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'DiffingBlurSlider', '整体遮罩混合', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Occluder
        row = row + 1
        self.widget['OccluderSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'OccluderSwitch', '自动遮罩', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['OccluderSlider'] = GE.Slider2(self.layer['parameters_frame'], 'OccluderSlider', '大小', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Mask XSeg
        row = row + 1
        self.widget['DFLXSegSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'DFLXSegSwitch', 'DFL 遮罩', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['DFLXSegSlider'] = GE.Slider2(self.layer['parameters_frame'], 'DFLXSegSlider', '大小', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['OccluderBlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'OccluderBlurSlider', '变形子', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # FinalBlurSlider
        row = row + 1
        self.widget['FinalBlurSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'FinalBlurSwitch', '最终模糊开关', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['FinalBlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'FinalBlurSlider', '最终面部模糊', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Overall MaskBlendSlider
        row = row + 1
        self.widget['BlendSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BlendSlider', '整体罩混合', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # DFL RCT Color Transfer
        row = row + 1
        self.widget['DFLRCTColorSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'DFLRCTColorSwitch', 'DFL RCT 色彩转移', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady+5)

        # DFL Load only one Model
        row = row + 1
        self.widget['DFLLoadOnlyOneSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'DFLLoadOnlyOneSwitch', 'DFL 在内存中只保留单个模型', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady+5)

        # DFL AMP Morph Factor
        row = row + 1
        self.widget['DFLAmpMorphSlider'] = GE.Slider2(self.layer['parameters_frame'], 'DFLAmpMorphSlider', 'DFL AMP形态因子', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady+5, 0.62)

        # CLIP
        row = row + 1
        self.widget['CLIPSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'CLIPSwitch', '基于文本的遮罩', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['CLIPTextEntry'] = GE.Text_Entry(self.layer['parameters_frame'], 'CLIPTextEntry', '文本', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['CLIPSlider'] = GE.Slider2(self.layer['parameters_frame'], 'CLIPSlider', '数', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        #Restore Eyes
        row = row + 1
        self.widget['RestoreEyesSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'RestoreEyesSwitch', '保留眼睛', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['RestoreEyesSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesSlider', '眼睛混合', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['Eyes_Mouth_BlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'Eyes_Mouth_BlurSlider', '眼睛嘴巴遮罩模糊', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreEyesFeatherSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesFeatherSlider', '眼睛边缘羽化', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreEyesSizeSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesSizeSlider', '眼睛区域大小', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreEyesRadiusFactorXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesRadiusFactorXSlider', '眼睛半径系数: X', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreEyesRadiusFactorYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesRadiusFactorYSlider', '眼睛半径系数: Y', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreEyesSpacingOffsetSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesSpacingOffsetSlider', '眼间距偏移', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreEyesXoffsetSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesXoffsetSlider', '眼睛抵消: X', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreEyesYoffsetSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreEyesYoffsetSlider', '眼睛抵消: Y', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        #Restore Mouth
        row = row + 1
        self.widget['RestoreMouthSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'RestoreMouthSwitch', '保留嘴巴', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['RestoreMouthSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreMouthSlider', '嘴巴混合', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreMouthFeatherSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreMouthFeatherSlider', '嘴巴边缘羽化', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreMouthSizeSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreMouthSizeSlider', '嘴巴大小', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreMouthRadiusFactorXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreMouthRadiusFactorXSlider', '嘴巴半径系数: X', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreMouthRadiusFactorYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreMouthRadiusFactorYSlider', '嘴巴半径系数: Y', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreMouthXoffsetSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreMouthXoffsetSlider', '嘴巴抵消: X', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['RestoreMouthYoffsetSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestoreMouthYoffsetSlider', '嘴巴抵消 Y', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # FaceParser - Face
        row = row + 1
        self.widget['FaceParserSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'FaceParserSwitch', '人脸解析', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)

        #Face Background & Blurs & Neck
        row = row + 1
        self.widget['BGParserBlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BGParserBlurSlider', '背景模糊', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['ParserBlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ParserBlurSlider', '人脸解析模糊', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['FaceParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'FaceParserSlider', '背景', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['NeckParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'NeckParserSlider', '颈部', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)

        #Eyebrows
        row = row + 1
        self.widget['LeftEyeBrowParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'LeftEyeBrowParserSlider', '左眉', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['RightEyeBrowParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RightEyeBrowParserSlider', '右眉', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)

        #Eyes
        row = row + 1
        self.widget['LeftEyeParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'LeftEyeParserSlider', '左眼', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['RightEyeParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RightEyeParserSlider', '右眼', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)

        #Nose and Mouth
        row = row + 1
        self.widget['NoseParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'NoseParserSlider', '鼻子', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62,40)
        row = row + 1
        self.widget['MouthParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'MouthParserSlider', '嘴巴', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62,40)

        #Lips
        row = row + 1
        self.widget['UpperLipParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'UpperLipParserSlider', '上嘴唇', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['LowerLipParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'LowerLipParserSlider', '下嘴唇', 3, self.update_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)

        # Autocolor
        row = row + 1
        self.widget['AutoColorSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'AutoColorSwitch', '自动着色', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['AutoColorTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'AutoColorTypeTextSel', '转移类型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['AutoColorSlider'] = GE.Slider2(self.layer['parameters_frame'], 'AutoColorSlider', '自动着色混合', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Jpeg Compression
        row = row + 1
        self.widget['JpegCompressionSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'JpegCompressionSwitch', 'Jpeg压缩', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['JpegCompressionSlider'] = GE.Slider2(self.layer['parameters_frame'], 'JpegCompressionSlider', 'Jpeg 值', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Color Adjustments
        row = row + 1
        self.widget['ColorSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'ColorSwitch', '色彩调整', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['ColorRedSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorRedSlider', '红', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorGreenSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorGreenSlider', '绿', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorBlueSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorBlueSlider', '蓝', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorBrightSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorBrightSlider', '亮度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorContrastSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorContrastSlider', '对比度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorSaturationSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorSaturationSlider', '饱和度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorSharpnessSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorSharpnessSlider', '锐化', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorHueSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorHueSlider', '色调', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ColorGammaSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorGammaSlider', '伽马', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['NoiseSlider'] = GE.Slider2(self.layer['parameters_frame'], 'NoiseSlider', '噪点', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # KPS Adjustment and scaling
        row = row + 1
        self.widget['FaceAdjSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'FaceAdjSwitch', '输入面部调整', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        ...
        row = row + 1
        self.widget['KPSScaleSlider'] = GE.Slider2(self.layer['parameters_frame'], 'KPSScaleSlider', 'KPS - 缩放', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['FaceScaleSlider'] = GE.Slider2(self.layer['parameters_frame'], 'FaceScaleSlider', '面部缩放', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Face Likeness
        row = row + 1
        self.widget['FaceLikenessSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'FaceLikenessSwitch', '面部相似度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['FaceLikenessFactorSlider'] = GE.Slider2(self.layer['parameters_frame'], 'FaceLikenessFactorSlider', '系数', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Threshhold
        row = row + 1
        self.widget['ThresholdSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ThresholdSlider', '相似度阈值', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Cats and Dogs
        row = row + 1
        self.widget['DetectTypeTextSel'] = GE.TextSelectionComboBox(self.layer['parameters_frame'], 'DetectTypeTextSel', '检测类型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.62, 150)
        row = row + 1
        self.widget['DetectScoreSlider'] = GE.Slider2(self.layer['parameters_frame'], 'DetectScoreSlider', '检测分数', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Similarity
        row = row + 1
        self.widget['SimilarityTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'SimilarityTypeTextSel', '相似类型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.62)

        # Auto Rotation
        row = row + 1
        self.widget['AutoRotationSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'AutoRotationSwitch', '自动旋转', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)

        # Landmarks Detection
        row = row + 1
        self.widget['LandmarksDetectionAdjSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'LandmarksDetectionAdjSwitch', '地标检测调整', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        self.widget['LandmarksAlignModeFromPointsSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'LandmarksAlignModeFromPointsSwitch', '起点', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 30, 40)
        row = row + 1
        self.widget['LandmarksDetectTypeTextSel'] = GE.TextSelectionComboBox(self.layer['parameters_frame'], 'LandmarksDetectTypeTextSel', '地标检测类型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.62, 150)
        row = row + 1
        self.widget['LandmarksDetectScoreSlider'] = GE.Slider2(self.layer['parameters_frame'], 'LandmarksDetectScoreSlider', '地标检测分数', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['ShowLandmarksSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'ShowLandmarksSwitch', '显示地标', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)

        # Face Landmarks Position
        row = row + 1
        self.widget['LandmarksPositionAdjSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'LandmarksPositionAdjSwitch', '5 地标位置调整', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady)
        row = row + 1
        ...
        row = row + 1
        self.widget['EyeLeftXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'EyeLeftXSlider', '左眼:   X', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['EyeLeftYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'EyeLeftYSlider', '左眼:   Y', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['EyeRightXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'EyeRightXSlider', '右眼:   X', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['EyeRightYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'EyeRightYSlider', '右眼:   Y', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['NoseXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'NoseXSlider', '鼻子:   X', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['NoseYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'NoseYSlider', '鼻子:   Y', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['MouthLeftXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'MouthLeftXSlider', '嘴角左侧:   X', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['MouthLeftYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'MouthLeftYSlider', '嘴角左侧:   Y', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['MouthRightXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'MouthRightXSlider', '嘴角右侧:   X', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)
        row = row + 1
        self.widget['MouthRightYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'MouthRightYSlider', '嘴角右侧:   Y', 3, self.update_face_landmarks_data, 'parameter', 300, 20, row, 0, padx, pady, 0.62, 40)

        row = row + 1
        self.widget['RecordTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'RecordTypeTextSel', '记录类型', 3, self.update_data, 'parameter', 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['VideoQualSlider'] = GE.Slider2(self.layer['parameters_frame'], 'VideoQualSlider', '视频质量', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['AudioSpeedSlider'] = GE.Slider2(self.layer['parameters_frame'], 'AudioSpeedSlider', '音频播放速度', 3, self.update_data, 'parameter', 398, 20, row, 0, padx, pady, 0.62)
        row = row + 1
        self.widget['MergeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'MergeTextSel', '合并计算', 3, self.select_input_faces, 'merge', '', 398, 20, row, 0, padx, pady, 0.62)
        ...
        # Providers Priority
        row = row + 1
        self.widget['FaceEditorTypeTextSel'] = GE.TextSelection(self.layer['parameters_face_editor_frame'], 'FaceEditorTypeTextSel', '脸部编辑器类型', 3, self.update_face_editor_data, 'parameter_face_editor', 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60)
        row = row + 1
        self.widget['FaceEditorIDSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'FaceEditorIDSlider', '脸部编辑器ID: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60)
        row = row + 1
        self.widget['CropScaleSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'CropScaleSlider', '裁剪缩放: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['EyesOpenRatioSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'EyesOpenRatioSlider', '眼睛闭上 <--> 张开比例: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['LipsOpenRatioSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'LipsOpenRatioSlider', '嘴唇闭上 <-->  张开比例: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['HeadPitchSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'HeadPitchSlider', '头X轴俯仰角: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['HeadYawSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'HeadYawSlider', '头Y轴偏航角: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['HeadRollSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'HeadRollSlider', '头Z轴翻滚角: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['XAxisMovementSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'XAxisMovementSlider', 'X轴移动: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['YAxisMovementSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'YAxisMovementSlider', 'Y轴移动: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['ZAxisMovementSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'ZAxisMovementSlider', 'Z轴移动: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['MouthPoutingSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'MouthPoutingSlider', '噘嘴: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['MouthPursingSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'MouthPursingSlider', '缩嘴: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['MouthGrinSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'MouthGrinSlider', '咧嘴: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['LipsCloseOpenSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'LipsCloseOpenSlider', '嘴唇闭合 <--> 打开大小: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['MouthSmileSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'MouthSmileSlider', '嘴巴微笑: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['EyeWinkSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'EyeWinkSlider', '眨眼: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['EyeBrowsDirectionSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'EyeBrowsDirectionSlider', '眉毛方向 : ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['EyeGazeHorizontalSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'EyeGazeHorizontalSlider', '眼注视水平: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
        row = row + 1
        self.widget['EyeGazeVerticalSlider'] = GE.Slider2(self.layer['parameters_face_editor_frame'], 'EyeGazeVerticalSlider', '眼睛凝视垂直: ', 3, self.update_face_editor_data, 'parameter_face_editor', 398, 20, row, 0, padx, pady, 0.60, 40)
 ...
 ######### Options
        ...
        self.status_left_label = tk.Label(bottom_frame, style.donate_1, cursor="hand2", text="帮助文档")
        self.status_left_label.grid( row = 0, column = 0, sticky='NEWS')
        self.status_left_label.bind("<Button-1>", lambda e: self.callback("https://ailive.ai-yy.com/help"))

        # self.status_label = tk.Label(bottom_frame, style.donate_1, text="QQ: 66861140")
        # self.status_label.grid( row = 0, column = 1, sticky='NEWS')
        # self.status_label.bind("<Button-1>", lambda e: self.callback(""))

        self.license_status_label = tk.Label(bottom_frame, style.donate_1, text="", anchor='e')
        self.license_status_label.grid(row=0, column=2, sticky='NEWS')
        ...
        self.version_label.grid(row=0, column=1, sticky='NEWS')
...
# refactor - make sure files are closed
    ...
    def initialize_gui( self ):
        ...
        try:
            data_json_file = open("config/data.json", "r")
        except:
        ...
        self.video_image = cv2.cvtColor(cv2.imread(get_media_path('splash_next.png')), cv2.COLOR_BGR2RGB)
        self.resize_image()
    ...
    def load_dfl_input_models(self):
        text_font = font.Font(family="Microsoft YaHei", size=10)
...
# Refactor this, doesn't seem very efficient
    ...
    def check_and_show_update(self):
        """检查并显示更新提示"""
        try:
            # 确保 version_label 已创建
            if not hasattr(self, 'version_label') or self.version_label is None:
                logging.debug("Version label not created yet")
                return
